...
if (isset($_POST['bayar'])) {
    ...
    //proses simpan
    $query = "INSERT INTO pembayaran_buku (id_petugas, nisn, tgl_bayar, id_buku, jumlah_bayar) VALUES ('$id_petugas', '$nisn', '$tgl_bayar', '$id_buku', '$jumlah_bayar')";
    $result = mysqli_query($conn, $query);
    if ($result) {
        echo "<script>alert('Pembayaran buku berhasil!')
                    document.location = 'pembayaran_buku.php?nisn=$nisn&cari=';
                </script>";
    } else {
        echo "<script>alert('Pembayaran buku gagal!')
                    document.location = 'pembayaran_buku.php?nisn=$nisn&cari=';
                </script>";
    }
}
...
<div class="card mb-4">
    <div class="card-header bg-success py-3 d-flex justify-content-between">
        <h5 class="m-0 font-weight-bold text-light">Transaksi Pembayaran Buku</h5>
    </div>
    <div class="card-body">
        <?php
        $nisn = $_GET['nisn'];
        $kekurangan = $_GET['kekurangan'];
        $query = "SELECT * FROM siswa, buku, kelas
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="id_buku" class="form-control" value="<?= $row['id_buku'] ?>"/>
